...
import (
	...
	"github.com/Golang_Rest_API/app/dbutils"
...
//ScheduleResource link both trains and stations
...
func (t *TrainResource) Register(container *restful.Container) {
	...
	ws.Route(ws.GET("/all").To(t.getTrains))
	container.Add(ws)

}

func (t TrainResource) getTrains(request *restful.Request, response *restful.Response) {
	log.Println("Getting all trains")

	rows, err := DB.Query("SELECT ID, DRIVER_NAME, OPERATING_STATUS FROM train LIMIT $1", 3)
	defer rows.Close()
	var trains []TrainResource
	if err != nil {
		log.Println(err)
		response.AddHeader("Content-Type", "text/plain")
		response.WriteErrorString(http.StatusNotFound, "No train registered.")
	}
	for rows.Next() {

		err = rows.Scan(&t.ID, &t.DriverName, &t.OperatingStatus)
		if err != nil {
			fmt.Println("Error !")
		}
		trains = append(trains, t)
	}
	response.WriteEntity(trains)
	err = rows.Err()
	if err != nil {
...
func (t TrainResource) getTrain(request *restful.Request, response *restful.Response) {
	log.Println("Getting one train")
	id := request.PathParameter("train-id")
	err := DB.QueryRow("select ID, DRIVER_NAME, OPERATING_STATUS FROM train where id=?", id).Scan(
		&t.ID, &t.DriverName, &t.OperatingStatus,
	)
	if err != nil {
		log.Println(err)
		response.AddHeader("Content-Type", "text/plain")
...
func (t TrainResource) createTrain(request *restful.Request, response *restful.Response) {
	log.Println("Registering trains")
...
func main() {
	...
	log.Printf("start listening on locahost:8000")
